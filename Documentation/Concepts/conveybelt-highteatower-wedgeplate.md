...
### Visual Appearance Diagram

This shows the scene as it appears in-game with all UI elements:

```
  ┌─ HUD ──────────────────────────────────────────────────────────┐
  │  Conveyor Shift   [████████░░] 6/12    Streak: 3    ❌ 1      │
  └────────────────────────────────────────────────────────────────┘
...
```
  ╱╲╱╲╱╲╱╲   = Filled tower slots (wedge plates stacked in tower)
  ╱╲······   = Partially filled tier (· = empty slot)
  ─────────   = Tower tier separator plate
      │       = Tower pole
                (tower stand at base, not shown)

  ♥ 4/12     = Heart bubble (light pink at low counts, bright red near 12)
  💬 Chat?   = Lavender call bubble (clickable, sends barista)
  ☕          = Barista sprite (walks to tables on call bubble click)
  🧽          = Sponge on messy table (click to clean)

  ▷ 🍰 ▷    = Wedge plates moving clockwise on belt
  ○           = Belt corner markers

  🧑 👩 👨 etc = Customer avatars seated around tables
```
...
```
              ╭─────────╮
              │ 💬 Chat?│    ← call bubble (lavender, offset 20px)
              ╰────┬────╯
              ╭────┴────╮
              │ ♥  7/12 │    ← heart bubble (offset 4px, color = lerp)
              ╰────┬────╯
                   ▼
         🧑  ┌──────────┐  👩     ← seats (36px circles, elliptical orbit)
              │ ╱╲╱╲╱╲╱╲│         ← tier 3 (top)
              │ ─────────│         ← separator
              │ ╱╲╱╲╱╲╱╲│         ← tier 2
              │ ─────────│         ← separator
              │ ╱╲╱╲╱╲·· │         ← tier 1 (base, partially filled)
              │    │     │         ← pole
              │  [====]  │         ← stand
              └──────────┘
                80x80px
...
### Plate on Belt Close-Up

```
     ┌──────┐
     │  ╱╲  │   40x40px wedge plate
     │ ╱🍰╲ │   category-colored triangle (clip-path)
     │╱────╲│   bottom border matches category
     └──────┘
      Latte      label (shown on hover, 0.45rem)
```

### Tower Completion Sequence

```
  ORDERING              SERVED (2s)         EATING (8s)          MESSY
  ┌────────┐            ┌────────┐          ┌────────┐          ┌────────┐
  │╱╲╱╲╱╲╱╲│           │╱╲╱╲╱╲╱╲│  ✨     │╱╲╱╲╱╲╱╲│  ❤️     │  🧽    │
  │────────│            │────────│  glow    │────────│  pulse   │        │
  │╱╲╱╲╱╲╱╲│           │╱╲╱╲╱╲╱╲│         │╱╲╱╲╱╲╱╲│  x5     │        │
  ...
  │ [====] │            │ [====] │         │ [====] │         │        │
  └────────┘            └────────┘          └────────┘          └────────┘
  ♥ bright red          green glow          hearts every        click sponge
  tower .complete       overlay             1.5s for 8s         to clean
```

### Guest Order Board Overlay
...
```
  ...
  │  ╰──╯  "We're 7/12 done…"          │
  │─────────────────────────────────────│
  │  🍰  Strawberry Cake ×2   [✓ done] │  ← green border
  │  ☕  Latte ×3              [✓ done] │
  │  🥪  Club Sandwich ×1     [✓ done] │
  │  🧁  Blueberry Muffin ×2  [needed] │  ← peach border
  │  🥗  Garden Salad ×1      [needed] │
  │  ⬜  3 empty slots left    [ open ] │  ← dashed, faded
  │─────────────────────────────────────│
  │  [███████████░░░░░] ♥ 7/12         │  ← summary bar
  │─────────────────────────────────────│
  │  ╭──────────────────────────────╮   │
  │  │"I can't have caffeine…       │   │  ← chat bubble (blush)
  │  │  anything gentle?"           │   │
  │  ╰──────────────────────────────╯   │
  │  ┌──────────────┐┌─────────────┐    │
  │  │ Herbal teas! ││ Small coffee││    │  ← choice buttons
  │  └──────────────┘└─────────────┘    │
  │                                     │
  │         [ Done ]                    │  ← dusty-rose gradient
  └─────────────────────────────────────┘
```
